...
class SimpleDNN(nn.Module):
    """Simple 2-layer network: input -> output (logits).

    This is effectively a linear classifier (no hidden layer). Kept
    to provide logits for CrossEntropyLoss.
    """
    ...
    def forward(self, x):
        return self.fc(x)  # logits for CrossEntropyLoss
...
def train_model(hidden1, hidden2, lr, num_epochs=num_epochs, batch_size=batch_size, verbose=False):
    ...
    # hidden1/hidden2 are ignored for the 2-layer model; kept for compatibility
    model = SimpleDNN(input_size, output_size)
...
def train_model_with_history(hidden1, hidden2, lr, num_epochs=num_epochs, batch_size=batch_size, verbose=False,
    ...
    # hidden1/hidden2 ignored; use 2-layer linear model
    model = SimpleDNN(input_size, output_size)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.SGD(model.parameters(), lr=lr)
...
def sweep_hyperparams(lr_list, hidden1_list, hidden2_list=None, num_epochs_local=5, batch_size_local=32):
    """Simple hyperparameter sweep over learning rates and hidden sizes.

    hidden2_list is accepted for compatibility but ignored (3-layer net).
    Returns list of tuples: (lr, h1, h2_placeholder, accuracy)
    """
    results = []
    for lr in lr_list:
        for h1 in hidden1_list:
            # choose a placeholder for h2 to keep previous calling conventions
            h2_placeholder = hidden2_list[0] if hidden2_list else 0
            acc = train_model(h1, h2_placeholder, lr, num_epochs=num_epochs_local, batch_size=batch_size_local)
            results.append((lr, h1, h2_placeholder, acc))
